...
public class CFGGenerationforBranchvsStatementvsBasispathCoverage implements ICFGGeneration {
	final static UETLogger logger = UETLogger.get(CFGGenerationforBranchvsStatementvsBasispathCoverage.class);
	/**
	...
	public static void main(String[] args) throws Exception {
		ProjectParser parser = new ProjectParser(new File("/Users/ducanhnguyen/Documents/akautauto/datatest/duc-anh/cov"));
		parser.setExpandTreeuptoMethodLevel_enabled(true);
		INode function = Search
				.searchNodes(parser.getRootTree(), new FunctionNodeCondition(), "test(int)").get(0);
...
//		clone.setAST(Utils.getFunctionsinAST(normalizedCoverage.toCharArray()).get(0));
	...
	private void visitStatement(IASTStatement stm, ICfgNode begin, ICfgNode end, ICfgNode _break, ICfgNode _continue,
		...
		if (stm instanceof IASTIfStatement) {
			...
			IASTNode astCond = stmIf.getConditionExpression();
			...
			IASTNode astDecla = ((CPPASTIfStatement) stmIf).getConditionDeclaration();
			...
			if (astCond != null && astCond instanceof IASTExpression)
				visitCondition((IASTExpression) astCond, begin, afterTrue, afterFalse, parent, ICFGGeneration.IF_FLAG);
			else if (astDecla != null && astDecla instanceof IASTDeclaration)
				visitDeclaraInControlCondition((IASTDeclaration) astDecla, begin, afterTrue, afterFalse, parent, ICFGGeneration.IF_FLAG);
