...
public class RelatedConstraintsChecker implements IConstraintsChecker {
	final static UETLogger logger = UETLogger.get(RelatedConstraintsChecker.class);
	...
	public boolean check() {
		...
		IASTNode astLast = Utils.convertToIAST(lastConstraint.getConstraint());
		...
		if (constraints.getNumofConditions() == 1)
			...
			for (int i = 0; i < constraints.getNumofConditions() - 1; i++) {
				IASTNode astCon = Utils.convertToIAST(constraints.getElementAt(i));
				if (astCon != null)
