...
import entities.cfg.NormalCfgNode;
import entities.cfg.SimpleCfgNode;
import entities.testpath.*;
...
//		System.out.println(new UnaryNormalizer().rewrite(specialTestpath).toReducedString());
	...
	private void parseNormalNode(NormalCfgNode currentCfgNode, ITestpathInCFG newTestpath, boolean nextIsTrue) {
		...
		for (String newStm : pre)
			runFirst.add(new SimpleCfgNode(Utils.convertToIAST(newStm)));

		// Create after statement
		List<String> after = shortenNorm.getAfter();
		for (String newStm : after)
			runLast.add(new SimpleCfgNode(Utils.convertToIAST(newStm)));
	...
	private void normalizeAssignment(NormalCfgNode newConditionNode) {
		...
		newConditionNode.setAst(Utils.convertToIAST(newContent));
	}
	...
	private void normalizeSingleBoolVariable(NormalCfgNode newConditionNode) {
		 ...
		 */
		List<CPPASTIdExpression> ids = ASTUtils.getIds(newConditionNode.getAst());
		List<ICPPASTArraySubscriptExpression> arrayItems = ASTUtils.getArraySubscriptExpression(newConditionNode.getAst());
		...
		newConditionNode.setAst(Utils.convertToIAST(condition));
	}
