...
//		 EnumNode enumNode = (EnumNode) Search.searchNodes(parser.getRootTree(),
//                 new EnumNodeCondition(), "Color").get(0);
//
//		 System.out.println(enumNode.getAST().getRawSignature());
//
//		 List<String[]> list = enumNode.getAllEnumItems();
//		 for (String[] s : enumNode.getAllEnumItems())
//		 System.out.println(s[0] + " : " + enumNode.getValueOfEnumItem(s[0]));
//	 }
			...
			if (enumItem instanceof CPPASTEnumerator) {
				if (enumItem.getChildren().length == 2) {
					...
					if (!values.isEmpty()) {
						...
						int lastItemVal = Integer.parseInt(lastItem[VALUE_ENUM_ITEM]);
