...
import entities.compiler.message.error_tree.node.IUndeclaredFunctionErrorNode;
import entities.compiler.message.error_tree.node.UndeclaredErrorNode;
...
//        source = String.format("%s {}", source);
    ...
    private List<FunctionNode> findExistFunction() {
        ...
        if (!(errorNode.getLocation() instanceof StructureNode)) {
            ProjectNode projectNode = null;// Environment.getInstance().getProjectNode();
            List<INode> functions = Search.searchNodes(projectNode, new FunctionNodeCondition());

            INode dataRoot = null;// Environment.getInstance().getUserCodeRoot();
            functions.addAll(Search.searchNodes(dataRoot, new FunctionNodeCondition()));
